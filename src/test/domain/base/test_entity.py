...
class TestEntity:
    ...
    def test_to_json_given_entity_with_dict_returns_dict(self):
        ...
        assert entity_with_dict.to_json() == {
            "my_dict": {
                "key": "value",
                "foo": "bar"
            }
    ...
    def test_from_json_empty_entity(self):
        empty_entity = EmptyEntity().from_json({"this_field": "won't exist"})
        assert empty_entity.__dict__ == {}

    def test_from_json_entity_with_list(self):
        entity_with_list = EntityWithList().from_json({"my_list": ["new list!", "with items!"], "this_field": "won't "
                                                                                                              "exist"})
    ...
    def test_from_json_entity_with_child(self):
        entity_with_child = EntityWithChild().from_json({"my_field": "Nice", "my_child": {"my_field": "One"}})
    ...
    def test_from_json_entity_with_more_child_and_default_values_are_applied(self):
        entity_with_more_child = EntityWithMoreChild().from_json(
    ...
    def test_from_json_entity_with_dict_dict_is_restored(self):
        entity_with_dict = EntityWithDict().from_json({"my_dict": {"new": "dictionary"}})
